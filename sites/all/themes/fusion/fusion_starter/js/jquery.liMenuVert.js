 ...
 * 23,05,2013
 */
	$dr.fn.liMenuVert = function(params){
		...
		return this.each(function(){
			...
			menuSubSub = $dr('ul' , menuWrap);
		
			menuSubSub.each(function(){
				var 
				mArrowRight = $dr('<div>').addClass('arrow-right'),
				mSubSub = $dr(this),
				mSubList = mSubSub.closest('li'),
				mSubLink = mSubList.children('a').append(mArrowRight),
				mSubArrow = $dr('<div>').addClass('arrow-left').prependTo(mSubSub),
				mSubArrow2 = $dr('<div>').addClass('arrow-left2').prependTo(mSubSub),
				mFuncId = function(){},
				mFuncIdOut = function(){};
				
				mSubList.on('mouseenter',function(){
					clearTimeout(mFuncIdOut);
					mHoverLink = $dr(this).children('a');
